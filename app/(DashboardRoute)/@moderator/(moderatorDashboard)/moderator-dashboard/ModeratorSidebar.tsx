...
import {
    ...
    Users, LightbulbIcon, Shield, Activity, Settings, MessageCircle, Tag
} from "lucide-react"
...
const NAV_ITEMS = [
    ...
    { id: "ideas",          label: "Ideas Management",   icon: Shield },
    { id: "categories",     label: "Categories",         icon: Tag },
    { id: "my-ideas",       label: "My Ideas",           icon: LightbulbIcon },

    { id: "create",         label: "Create Idea",        icon: Activity }, // Keep activity, looks good
    { id: "purchased",      label: "Purchased Ideas",    icon: Shield }, // Changed to shield earlier, let's use Activity/Shield mix or stick to Member icons
    ...
    return (
        <>
            ...
            >
                ...
                {/* Moderator badge + user */}
                <div className="border-b border-border dark:border-border px-6 py-4">
                    <div className="flex items-center gap-3">
                        <div className="relative flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-green-600 text-sm font-black text-primary-foreground">
                            {user?.name?.[0]?.toUpperCase() || "M"}
                            <Shield className="absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full bg-green-950 text-accent-foreground p-0.5" />
                        </div>
                        <div>
                            <p className="text-sm font-semibold text-foreground dark:text-white">{user?.name || "Moderator"}</p>
                    ...
                    <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-accent/30 bg-accent/10 px-2.5 py-0.5">
                        ...
                        <span className="text-xs font-semibold text-accent-foreground">Moderator</span>
                    </div>
                </div>
