...
export default function IdeasManagement({ accessToken }: { accessToken: string }) {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-black text-foreground dark:text-white">Ideas Management</h2>
                <p className="mt-1 text-sm text-muted-foreground dark:text-green-200/50">Oversee all platform ideas, approve submissions, or provide rejection feedback.</p>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-muted-foreground dark:text-green-100">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {ideas.map(idea => (
                                <tr key={idea.id} className="transition-colors hover:bg-muted dark:hover:bg-white/5">
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-foreground dark:text-white line-clamp-1">{idea.title}</div>
                                        <div className="mt-0.5 text-xs text-muted-foreground dark:text-green-200/50">
                                            {idea.category?.name || "Uncategorized"} • {new Date(idea.createdAt).toLocaleDateString()}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-muted dark:bg-muted text-[10px] font-bold text-foreground dark:text-white">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${
                                            idea.status === "APPROVED" ? "border-primary/20 bg-primary/10 text-primary dark:text-emerald-400" :
                                            idea.status === "REJECTED" ? "border-destructive/20 bg-destructive/10 text-destructive dark:text-destructive" :
                                            idea.status === "UNDER_REVIEW" ? "border-accent/20 bg-accent/10 text-accent-foreground dark:text-accent-foreground" :
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link 
                                                href={`/ideas/${idea.id}`}
                                                className="rounded-lg p-2 text-muted-foreground dark:text-green-200/50 hover:bg-muted dark:hover:bg-white/10 hover:text-foreground dark:hover:text-white transition-colors"
                                                title="View Idea"
                                            >
                                            ...
                                            {idea.status === "UNDER_REVIEW" && (
                                                <>
                                                    <button 
                                                        ...
                                                        className="rounded-lg p-2 text-primary/60 dark:text-emerald-400/60 hover:bg-primary/10 hover:text-primary dark:hover:text-emerald-400 transition-colors"
                                                        title="Approve Idea"
                                                    >
                                                    ...
                                                    <button 
                                                        ...
                                                        className="rounded-lg p-2 text-destructive/60 dark:text-destructive/60 hover:bg-destructive/10 hover:text-destructive dark:hover:text-destructive transition-colors"
                                                        title="Reject Idea"
                                                    >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl border border-border bg-background p-6 shadow-2xl">
                        ...
                        <textarea
                            ...
                            className="w-full rounded-xl border border-border bg-muted p-3 text-sm text-foreground placeholder:text-muted-foreground/30 focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/50"
                            rows={4}
                        />
                        <div className="mt-6 flex justify-end gap-3">
                            <button
                                onClick={() => setRejectId(null)}
                                className="rounded-xl px-4 py-2 text-sm font-semibold text-muted-foreground dark:text-green-200/60 hover:bg-muted dark:hover:bg-white/5 hover:text-foreground dark:hover:text-white transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="rounded-xl bg-destructive px-4 py-2 text-sm font-bold text-white shadow-lg shadow-red-500/20 transition-all hover:bg-destructive disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Reject Idea
                            </button>
                        </div>
